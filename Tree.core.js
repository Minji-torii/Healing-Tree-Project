var Tree = (function(){
	...
	<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
	const DEG2RAD = Math.PI/180;		//Cache: 업데이트시 자동으로 들어감
	...
	function Init(canvasID){
		if(Tree.gl != null) return Tree.gl;
	...
	class Util{
		...
		//example : Tree.Util.rgbArray("#FF0000","00FF00","#0000FF");
		static rgbArray(){
	...
	class Renderable extends Transform{
		constructor(vao,matName){
			...
			this.material = (matName != null && matName !== undefined)? Tree.Res.Materials[matName] : null;
		}

		setMaterial(matName){ this.material = Tree.Res.Materials[matName]; }
	...
	class CameraOrbit extends Transform{
		constructor(fov,near,far){
			...
			this.ubo = Tree.Res.Ubo[Tree.UBO_TRANSFORM];
		...
		//region Static Methods
			...
			static rotateVec3(qa,va,out){
				...
				var q = [qa[0],qa[1],qa[2]],		//Save the vector part of the Quaternion
					...
					d = Tree.Maths.Vec3.dot(q,v),	// U DOT V
					dq = Tree.Maths.Vec3.dot(q,q),	// U DOT U
					cqv = Tree.Maths.Vec3.cross(q,v,[0,0,0]);	// Cross Product for Q,V

					Tree.Maths.Vec3.scalarRev(q,2.0 * d,q);
					Tree.Maths.Vec3.scalarRev(v,s*s - dq,v);
					Tree.Maths.Vec3.scalarRev(cqv,2.0 * s,cqv);
	...
	*/
	...
	class ShaderBuilder{
		...
		prepareTextures(uName,TextureCacheName){
			...
			var loc = 0,tex = "";
			for(var i=0; i < arguments.length; i+=2){
				tex = Tree.Textures[arguments[i+1]];
	...
	class UBO{
		...
		static createTransformUBO(){
			return UBO.create(Tree.UBO_TRANSFORM,0,[ {name:"matProjection",type:"mat4"}, {name:"matCameraView",type:"mat4"}, {name:"posCamera",type:"vec3"} ]);
		}

		static create(blockName,blockPoint,ary){
			var bufSize = UBO.calculate(ary);
			Tree.Res.Ubo[blockName] = new UBO(blockName,blockPoint,bufSize,ary);
			//UBO.debugVisualize(Tree.Res.Ubo[blockName]);
			return Tree.Res.Ubo[blockName];
		}
	...
	class VAO{
		...
		static standardMesh(name,vertSize,aryVert,aryNorm,aryUV,aryInd,keepData){
			var rtn = {};
			VAO.create(rtn).floatArrayBuffer(rtn,"vert",aryVert,Tree.ATTR_POSITION_LOC,vertSize,0,0,true,keepData);
			rtn.count = rtn.buffers["vert"].count;

			if(aryNorm)	VAO.floatArrayBuffer(rtn,"norm",aryNorm,Tree.ATTR_NORM_LOC,3,0,0,true,keepData);
			if(aryUV)	VAO.floatArrayBuffer(rtn,"uv",aryUV,Tree.ATTR_UV_LOC,2,0,0,true,keepData);
	...
	var Renderer = (function(){
		...
		f.prepareNext = function(itm){
			...
			if(f.material.useModelMatrix) f.material.shader.setUniforms(Tree.UNI_MODEL_MAT_NAME,itm.worldMatrix);
