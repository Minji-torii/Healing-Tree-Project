...
TreeExt.Mesh = class{
	static triangleStrip(rLen,cLen,indAry,isLoop,doClose){ 
		//isLoop ties the left to the right, doClose is for paths that are closed shapes like a square
...
TreeExt.DynamicMesh = class extends Tree.Renderable{
	constructor(tVertComp,tVert,tIndex,matName){
		super({},matName);

		this.verts		= GLBuffer.float(null,tVertComp,tVert);
		this.drawMode	= Tree.gl.LINE_STRIP;
		this.visible	= false;

		//Create VAO with a buffer a predefined size buffer to dynamicly dump data in.
		Tree.Shaders.VAO.create(this.vao)
			.emptyFloatArrayBuffer(this.vao,"vert",this.verts.getBufferSize(),Tree.ATTR_POSITION_LOC,tVertComp,0,0,false)
		
		if(tIndex > 0){
			this.index 	= GLBuffer.element(null,1,tIndex);
			Tree.Shaders.VAO.emptyIndexBuffer(this.vao,"index",this.index.getBufferSize(),false);
			this.index.setRef(this.vao.buffers["index"].buf);
		}
		
		Tree.Shaders.VAO.finalize(this.vao,"TreeDynamicMesh");
	...
	draw(){
		if(this.vao.count > 0){
			...
			Tree.gl.drawElements(this.drawMode, this.vao.count, Tree.gl.UNSIGNED_SHORT, 0); 
			else
			Tree.gl.drawArrays(this.drawMode, 0, this.vao.count);
		}
	}
...
function GLBuffer(){
	var mGLArray = null, //UInt16Array;
		...
		mBufferSizeUsed = 0,
		mBufferType = Tree.gl.ARRAY_BUFFER,
		mComponentLength = 3,
		mAry = [];
	...
	return {
		...
		update:function(){
			...
			Tree.gl.bindBuffer(mBufferType,mBufferRef);

			//If data being push fits the existing buffer, send it up
			if(pushSize <= mBufferSize) Tree.gl.bufferSubData(mBufferType, 0, new mGLArray(mAry), 0, null);
			else{ //if not, we need to wipe out the data and resize the buffer with the new set of data.
				mBufferSize = pushSize;
				if(mBufferSizeUsed > 0) Tree.gl.bufferData(mBufferType, null, Tree.gl.DYNAMIC_DRAW); //Clean up previus data
				Tree.gl.bufferData(mBufferType, new mGLArray(mAry), Tree.gl.DYNAMIC_DRAW);
			}

			Tree.gl.bindBuffer(mBufferType,null); //unbind buffer
			mBufferSizeUsed = mAry.length;	
		}
	};
}

GLBuffer.float = function(bufRef,comLen,startSize){ return GLBuffer().setup(bufRef,Tree.gl.ARRAY_BUFFER,Float32Array,comLen,startSize); }
GLBuffer.element = function(bufRef,startSize){ return GLBuffer().setup(bufRef,Tree.gl.ELEMENT_ARRAY_BUFFER,Uint16Array,1,startSize); }
